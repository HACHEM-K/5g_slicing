...
thread_ue13 = threading.Thread(target=run_iperf, args=("rfsim5g-oai-nr-ue13", "192.168.70.145", "12.1.1.142", 5001, output_ue13, "UE 13"))
thread_ue14 = threading.Thread(target=run_iperf, args=("rfsim5g-oai-nr-ue14", "192.168.70.145", "12.1.1.143", 5001, output_ue14, "UE 14"))
thread_ue15 = threading.Thread(target=run_iperf, args=("rfsim5g-oai-nr-ue15", "192.168.70.145", "12.1.1.144", 5001, output_ue15, "UE 15"))
thread_ue16 = threading.Thread(target=run_iperf, args=("rfsim5g-oai-nr-ue16", "192.168.70.145", "12.1.1.145", 5001, output_ue16, "UE 16"))
...
with open('iperf_results.csv', 'w', newline='') as csvfile:
    ...
    # Write output for UE13
    for line in output_ue13:
        csvwriter.writerow([time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(start_time)), "rfsim5g-oai-nr-ue13", line])

    # Write output for UE14
    for line in output_ue14:
        csvwriter.writerow([time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(start_time)), "rfsim5g-oai-nr-ue14", line])

    # Write output for UE15
    for line in output_ue15:
        csvwriter.writerow([time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(start_time)), "rfsim5g-oai-nr-ue15", line])

    # Write output for UE16
    for line in output_ue16:
        csvwriter.writerow([time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(start_time)), "rfsim5g-oai-nr-ue16", line])

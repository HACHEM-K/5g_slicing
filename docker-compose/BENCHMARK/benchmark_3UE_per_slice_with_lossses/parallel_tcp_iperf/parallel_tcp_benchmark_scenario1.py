...
def benchmark_iperf_tcp(ue_container, dn_ip, bind_address, duration, port, start_time):
    ...
    # Run iperf3 client in UE container, specifying the bind address, port, and duration
    print(f"Running iperf3 client on port {port} in UE container for duration {duration} seconds...")
    iperf_output = run_command_in_container(ue_container, ['iperf3', '-c', dn_ip, '-B', bind_address, '-p', str(port), '-t', str(duration)])

...
def benchmark_iperf_tcp(ue_container, dn_container, dn_ip, bind_address, duration, interval):
    ...
    # Run iperf3 client in UE container, specifying the bind address, duration, and interval
    print(f"Running iperf3 client in UE container for duration {duration} seconds with interval {interval} seconds...")
    iperf_output = run_command_in_container(ue_container, ['iperf3', '-c', dn_ip, '-B', bind_address, '-t', str(duration), '-i', str(interval)])
...
for duration in durations:
    metrics = benchmark_iperf_tcp(ue_container, dn_container, dn_ip, bind_address, duration, interval)

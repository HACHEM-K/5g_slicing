...
def parse_iperf_output(output):
    ...
    # Bandwidth received (in Mbits/sec)
    if bandwidth_matches:
        bandwidth_received = float(bandwidth_matches[-1])  # Take the last bandwidth value
    else:
        bandwidth_received = 0

    return jitters, packet_loss, bandwidth_received
...
# Collect data
data = {'Bandwidth (Mbps)': [], 'Jitter (ms)': [], 'Packet Loss (%)': [], 'Received Bandwidth (Mbits/sec)': []}
...
for bw in bandwidths:
    ...
    jitters, packet_losses, bandwidth_received = parse_iperf_output(output)
    
    data['Bandwidth (Mbps)'].extend([bw] * len(jitters))
    data['Jitter (ms)'].extend(jitters)
    data['Packet Loss (%)'].extend(packet_losses)
    data['Received Bandwidth (Mbits/sec)'].extend([bandwidth_received] * len(jitters))
...
for i, bw in enumerate(bandwidths):
    ...
    q2 = y.median()
    q3 = y.quantile(0.75)
    axes[0].annotate(f'Q1: {q1:.2f}', xy=(i + 1, q1), xytext=(i + 1.3, q1),
                     arrowprops=dict(facecolor='black', shrink=0.05), fontsize=10)
    axes[0].annotate(f'Q2: {q2:.2f}', xy=(i + 1, q2), xytext=(i + 1.3, q2),
                     arrowprops=dict(facecolor='black', shrink=0.05), fontsize=10)
    axes[0].annotate(f'Q3: {q3:.2f}', xy=(i + 1, q3), xytext=(i + 1.3, q3),
                     arrowprops=dict(facecolor='black', shrink=0.05), fontsize=10)
    
    # Annotate received bandwidth
    bw_received = df['Received Bandwidth (Mbits/sec)'][df['Bandwidth (Mbps)'] == bw].iloc[0]
    axes[0].annotate(f'Received BW: {bw_received:.2f} Mbps', xy=(i + 1, q3 + 1), fontsize=10, ha='center')
...
for i, bw in enumerate(bandwidths):
    ...
    q2 = y.median()
    q3 = y.quantile(0.75)
    axes[1].annotate(f'Q1: {q1:.2f}', xy=(i + 1, q1), xytext=(i + 1.3, q1),
                     arrowprops=dict(facecolor='black', shrink=0.05), fontsize=10)
    axes[1].annotate(f'Q2: {q2:.2f}', xy=(i + 1, q2), xytext=(i + 1.3, q2),
                     arrowprops=dict(facecolor='black', shrink=0.05), fontsize=10)
    axes[1].annotate(f'Q3: {q3:.2f}', xy=(i + 1, q3), xytext=(i + 1.3, q3),
                     arrowprops=dict(facecolor='black', shrink=0.05), fontsize=10)
    
    # Annotate received bandwidth
    bw_received = df['Received Bandwidth (Mbits/sec)'][df['Bandwidth (Mbps)'] == bw].iloc[0]
    axes[1].annotate(f'Received BW: {bw_received:.2f} Mbps', xy=(i + 1, q3 + 5), fontsize=10, ha='center')
